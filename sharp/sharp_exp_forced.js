        ...
        for (const file of imageFiles) {
            ...
            try {
                ...
                switch (originalInfo.format) {
                    ...
                    case 'jpg':
                        optimizedImage = optimizedImage.jpeg({
                            quality: 100,  // Increased for better quality
                            progressive: true,
                            mozjpeg: true,
                            chromaSubsampling: '4:4:4',  // Preserve color quality
                            force: false  // Don't force if input is already optimal
                        });
                        break;
                    case 'png':
                ...
                if (optimizedBuffer.length > originalStats.size) {
                    // If optimized version is larger, keep original
                    await fs.copyFile(inputPath, outputPath);
...
const outputFolder = './optimized_images_forced';
const metricsPath = './optimization_metrics_forced.csv';
